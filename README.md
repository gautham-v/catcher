...
- Typing `[[` suggests notes, `[[note#` its headings, `[[note#^` its paragraphs and list items (a block without an id gets one written onto it, Obsidian's way), and `#` the vault's tags; **⏎** or **Tab** takes one, **Esc** dismisses. `autocomplete: no` turns it off.
