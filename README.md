...
- **⌥⏎** on a row in **^O** or **⇧^F** opens that note in a terminal split to the right and leaves this one where it is; **⌥⇧⏎** splits below, **⌃⏎** opens a tab, and **⌥click** does the same as **⌥⏎**. A link does it too: **⌥click** one opens its note in a split to the right and **⌃⌥click** in a new tab (a terminal never reports ⌘ with a click, and keeps ⇧click for its own selection), and a peek's bottom border lists the same four ways out — **⏎** here, **⌥⏎** right, **⌥⇧⏎** below, **⌃⏎** a tab — each one clickable. **⌥\\**, **⌥-** and **⌥T** (*open in split right / down / new tab* in the palette) open the linked note under the cursor, or the note you are in when the cursor is on no link. Every one of them makes a new split; none reuses one. Catcher asks the terminal: Ghostty 1.3+ (through AppleScript; macOS asks once whether catcher may control it), tmux, kitty and WezTerm. Elsewhere the status bar says so.
...
- The editor renders as you type. `**bold**`, `*italic*`, `` `code` ``, `==highlight==`, `[[link]]` and `[text](url)` are drawn on the keystroke that closes them; a `#tag` colours on its first letter and a heading on its `# `. A span shows its markup again only while the cursor is inside it, so the rest of the line stays rendered while you edit one word of it, and a selection reveals whatever it covers. What replaces a line rather than decorating it — an image, an embed, a rule, a table, a fence, `$$maths$$`, front matter — still shows its source until the cursor leaves it, since there is nowhere in the drawn form for a cursor to stand.
- `[[wikilinks]]` work like Obsidian's. **⌥⏎** follows one, **⌥P** peeks at it, **⌥click** opens it in a split beside this note, **⌥[** / **⌥]** go back and forward. A link to a note that does not exist yet is grey; following it makes the note beside the one you are in. `[[Note#Heading]]` reads as `Note › Heading` and lands on that heading; `[[Note#^id]]` lands on the line ending in ` ^id`; `[[#Heading]]` jumps within the note. `[text](note.md)` links open, count and get rewritten the same way. Renaming a heading fixes the `[[Note#Heading]]` links to it. `[[report.pdf]]` opens the file with the desktop. Links resolve through a note's front matter `aliases:` (or `alias:`) too. The reading view lists the notes that link here at the bottom, and the notes that mention this one's title without linking it.
...
- Mouse works: click, drag to select, scroll. A selection waits for **^C** — the drag itself never touches the clipboard, so you can type or paste over what you highlighted. Dragging in the reading view selects there too, and what it copies is the note's own markdown, not the page: the wrap, the hanging indent and the `·` a `-` is drawn as are the reading view's, and a bullet copied out of it goes back into another note as a bullet.
- Lists nest with **tab** and come back out with **⇧tab**, the way Obsidian's do: the item moves under the one above it, taking whatever is nested under it along, and an ordered list that starts there starts at `1.` while the list it left counts back up straight. A selection moves every item it touches, and keeps itself, so a run of them nests together and again. The first item of a list has nothing to nest under, so tab leaves it be.
- A nested list is **drawn where the page puts it, not where the file does**: it starts one step past the text of the item it hangs from — clear of that item's own wrapped lines, which is where the file's own indent would have left it — and a thin rule stands in each ancestor's text column, so the block a line belongs to is countable at a glance. The editor and the reading view draw it identically, rules and all, down every row a line wraps to. **Toggle list guides** in the palette (unbound, `key_list_guides`) turns the rules off and on, `list_guides: no` in the settings keeps them off; the nesting itself stays either way.
- Pasting keeps what was copied. **^V** takes the clipboard itself, and an image on it becomes a PNG attachment; the terminal's own **⌘V** arrives as a bracketed paste, which lands as text rather than as typing — a pasted `- item` keeps its marker instead of collecting another one from the list-continuation ⏎, and the whole paste undoes in one step. It is also why terminals stop asking whether a multi-line paste is safe. A pasted ordered list then joins the count of the one it landed in — five items copied out of another note come in as `1.`–`5.` and end up `8.`–`12.` — while a list written in the deliberate `1. 1. 1.` style is left exactly as it was.
- An ordered list stays in order as you edit it. Any edit that adds or removes a line counts the list it happened in through again — **⏎** in the middle of one, a delete that takes an item out, an item moved up or down — so the editor's numbers say what the page's do. An edit that only changes what a line says is left alone: a number typed over by hand is a number you meant.
