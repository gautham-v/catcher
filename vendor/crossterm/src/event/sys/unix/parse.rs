...
fn parse_cb(cb: u8) -> io::Result<(MouseEventKind, KeyModifiers)> {
    ...
    let kind = match (button_number, dragging) {
        ...
        // catcher: the side buttons, 8 (back) and 9 (forward). Upstream has
        // no `MouseButton` for them and drops the report, so they come
        // through as Left (back) and Right (forward) marked with HYPER, a
        // modifier no terminal ever reports on a mouse event. See
        // `side_button` in catcher's src/app.rs.
        (8, false) => MouseEventKind::Down(MouseButton::Left),
        (9, false) => MouseEventKind::Down(MouseButton::Right),
        // We do not support other buttons.
        _ => return Err(could_not_parse_event_error()),
    };

    let mut modifiers = KeyModifiers::empty();
    if matches!(button_number, 8 | 9) {
        modifiers |= KeyModifiers::HYPER;
    }
...
mod tests {
    ...
    #[test]
    fn test_parse_csi_sgr_mouse_side_buttons() {
        // catcher's patch: 128 is button 8 (back), 129 button 9 (forward)
        assert_eq!(
